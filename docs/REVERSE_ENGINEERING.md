...
```text
完整研究基线  Windows 微信 4.1.12.26
增量适配版本  Windows 微信 4.1.13.12（仅 WCDB 密钥入口）
业务模块  Weixin.dll
IDA 基址  0x180000000
方法      IDA 静态分析 + x64dbg 动态验证 + 离线数据库核对
```

除“WCDB 密钥边界”中单独列出的 `4.1.13.12` 结果外，本文函数地址、虚表和对象布局仍只对
`4.1.12.26` 有效。进程每次启动后都应重新取得模块基址：
...
| 微信版本 | IDA VA | RVA | 证据状态 |
|---|---:|---:|---|
| `4.1.12.26` | `0x1805DBF40` | `0x5DBF40` | 静态语义 + 动态参数 + 数据库验证 |
| `4.1.13.12` | `0x1805ECD00` | `0x5ECD00` | 静态锚点 + 内部语义片段 + `.pdata` 边界 |

两版入口都呈现相同的参数保存和配置安装形状：
...
当前 `chat_plugin` 读取 `Weixin.dll` 文件版本后选择上表 RVA，并在安装 Detour 前校验
入口签名。最干净的捕获点是函数入口；`4.1.12.26` 的配置构造器 `0x181465670` 会复制
key，然后对内部副本做 XOR 混淆。该构造器地址尚未在 `4.1.13.12` 重新定位。

`4.1.13.12` 静态样本：

```text
文件大小  196834352 bytes
SHA-256   E3240BF8A4D00593A4B3E6CE6C8B6AC26897622C27F410F6655C4EEE17CB3B6D
函数范围  RVA 0x5ECD00..0x5ECE31（来自 .pdata）
内部锚点  RVA 0x5ECD64 / 0x5ECD9D / 0x5ECE16
```

这三个内部锚点分别对应配置对象构造、安装 Cipher 配置和空值时移除同一配置；每个模式
在样本内均为单一命中。此处只声明静态兼容，动态参数和数据库交叉验证仍待执行。
...
| 微信版本 | 配置名 RVA | 注册函数 RVA | 全局配置键 RVA |
|---|---:|---:|---:|
| `4.1.12.26` | `0x892ABC8` | `0x1463DE0` | `0xAFE6E60` |
| `4.1.13.12` | `0x8CDD438` | `0x1488A60` | `0xB4D1A78` |

`4.1.13.12` 中配置名只有一个代码引用；注册函数同时引用配置名和全局键。密钥函数的安装
和移除路径又共同引用同一全局键。恢复步骤：
...
仓库提供离线复核脚本，会同时检查上述四段 AOB、`.pdata` 函数边界、配置名引用和同一
全局配置键：
...
- `4.1.13.12` 除 WCDB 密钥入口外的函数 RVA、虚表和对象布局；
- `4.1.13.12` WCDB 密钥入口的动态参数、数据库解密、数据库参数和表结构兼容性。
