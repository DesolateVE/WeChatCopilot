...
### 13.16 原生 WCDB 静态库与 C++ 读取验证

已固定并编译腾讯 WCDB `v2.1.16`（commit
`df808591b9f9a9ab42156006819c3550d5af13a3`），Windows x64 Release
配置包含 C++、SQLCipher、Zstd，并将全部目标统一为静态 CRT。

原生读取器 `src/native/weixin_wcdb_reader.cpp` 直接调用：

```text
Database(readOnly=true)
  -> setCipherKey(captured 32-byte input, 4096, Version4)
  -> setCompression(message_content/source)
  -> WINQ 查询
```
...
Msg_* tables:                      148
message rows:                   180606
compressed sample returned Text:  true
```

这与 Python/SQLCipher 基线完全一致，同时证明 WCDB 可以直接完成密钥配置和
压缩列透明解码。构建、运行以及 IDA FLIRT 签名流程见
`docs/WCDB_NATIVE_READER.md`。

IDA 9.3 SP2 的 `pcf`/`sigmake` 已将上述静态库转换为 FLIRT 签名：

```text
name:       WCDB 2.1.16 MSVC x64
modules:    7806
signature:  artifacts/ida/wcdb-2.1.16-msvc-x64.sig
installed:  E:\ida93sp2\sig\pc\wcdb-2.1.16-msvc-x64.sig
