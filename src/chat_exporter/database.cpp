...
std::string base64Encode(const unsigned char* data, const size_t size)
{
    ...
    if (CryptBinaryToStringA(data, static_cast<DWORD>(size), flags, nullptr, &characters) == FALSE)
    {
        throw std::runtime_error("cannot determine Base64 output size");
    }
    std::string encoded(characters, '\0');
    if (CryptBinaryToStringA(data, static_cast<DWORD>(size), flags, encoded.data(), &characters) == FALSE)
    {
...
{
ReadOnlyDatabase::ReadOnlyDatabase(const std::filesystem::path& path, const std::vector<unsigned char>& key,
    ...
    : path_(path), database_(pathUtf8(std::filesystem::absolute(path)), true)
{
    ...
    database_.setCipherKey(WCDB::UnsafeData::immutable(key.data(), key.size()), kCipherPageSize, WCDB::Database::CipherVersion::Version4);
    if (configureMessageCompression)
    {
        database_.setCompression(
                [](WCDB::Database::CompressionInfo& info)
                {
                    if (info.getTableName().hasPrefix("Msg_"))
                    {
                        info.addZSTDNormalCompressField(WCDB::Field("message_content", nullptr));
    ...
    {
        throw std::runtime_error("WCDB cannot open " + pathUtf8(path_) + ": " + wcdbText(database_.getError().getMessage()));
    }
}
...
bool ReadOnlyDatabase::tableExists(const std::string& tableName)
{
    const auto statement = WCDB::StatementSelect()
                                   ...
                                   .where(WCDB::Column("type") == "table" && WCDB::Column("name") == WCDB::BindParameter())
                                   .limit(1);
    bool found = false;
...
void ReadOnlyDatabase::forEach(const WCDB::Statement& statement, const std::vector<Binding>& bindings,
                               const std::function<void(WCDB::Handle&)>& callback)
{
    ...
    try
    {
        ...
        {
            throw std::runtime_error("WCDB prepare failed: " + wcdbText(handle.getError().getMessage()) + " | " +
        ...
        if (!succeeded)
        {
            throw std::runtime_error("WCDB step failed: " + wcdbText(handle.getError().getMessage()) + " | " +
                                     wcdbText(statement.getDescription()));
        }
        handle.finalize();
        handle.invalidate();
...
{
    return handle.getType(index) == WCDB::ColumnType::Null ? std::string() : wcdbText(handle.getText(index));
}
